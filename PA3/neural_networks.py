...
class linear_layer:
    ...
    def backward(self, X, grad):
        ...
        # its 1,N * N, output_dim
        # self.gradient['b'] = np.matmul(np.ones((1,self.gradient['b'].shape[0])),grad)
        self.gradient['b'] = grad.sum(axis=0) 
        #np.ones(grad.shape)*self.gradient['b']
    ...
    for module_name, module in model.items():
        ...
        if hasattr(module, 'params'):
            for key, _ in module.params.items():
                ...
                #think about the gradient, why plus right part, no need for changing gradient
                g = module.gradient[key] + _lambda * module.params[key]

                if _alpha > 0.0:
                    
                    # m means v in the formula 
                    # consider why g is formed by that
                    ...
                    # in this case might not be - may be + in this formula
